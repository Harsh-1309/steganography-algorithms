...
def bitchanger():
    for i in range(len(b_message)):
        if rangefinder(BC)=='range1':
            ...
            x=6
            while(rangefinder(s)!='range5'):
                BC_bin_mod=bitverifier(BC_bin_mod,x)
                s=int(BC_bin_mod,2)
                x=x-1 and x>=0
            image.itemset((n,j,0),s)
            i=i+1
        elif i>=len(b_message):
            print("end of string")
        break
